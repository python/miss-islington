...
async def test_ci_passed_with_one_core_dev_review_pr_is_merged():
    ...
    data = {
        "sha": sha,
        "commit": {
            "committer": {"login": "miss-islington",},
        }
    }
    event = sansio.Event(data, event='status',
                         delivery_id='1')
...
async def test_ci_passed_approved_by_non_core_dev_review_pr_is_not_merged():
    ...
    getitem = {
        ...
        "/teams/42/memberships/Mariatta": gidgethub.BadRequest(status_code=http.HTTPStatus(404))
        }
