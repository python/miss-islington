...
async def test_get_participants_different_creator_and_committer():
    gh = FakeGH(getitem={"/repos/python/cpython/pulls/5544": {
...
def test_pr_is_automerge():
    labels = [{"name": "CLA Signed"}, {"name": "automerge"}, {"name": "awaiting review"}]
...
async def test_get_pr_for_commit():
    ...
    gh = FakeGH(getitem={f"/search/issues?q=type:pr+repo:python/cpython+sha:{sha}": {
            "total_count": 1,
            "items": [
                {
                    "number": 5547,
...
async def test_get_pr_for_commit_not_found():
    sha = "f2393593c99dd2d3ab8bfab6fcc5ddee540518a9"
    gh = FakeGH(
        getitem={f"/search/issues?q=type:pr+repo:python/cpython+sha:{sha}": {
            "total_count": 0,
            "items": [
